...
| 상황 | 현재 동작 | 목표 동작 |
|------|-----------|-----------|
| 앱 포그라운드 | 배너 → `addNotificationReceivedListener` → 화면 이동 | 즉시 알람 화면 전환 + 알람음 + 진동 |
| 앱 백그라운드 | 배너 표시 → **사용자가 탭해야** 화면 이동 | 자동으로 전체화면 전환 |
| 잠금화면 | 배너만 표시 | 화면 켜지며 알람 화면 오버레이 |
...
### 2단계: Android Full-Screen Intent
